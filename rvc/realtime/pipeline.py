...
def create_pipeline(
    ...
    # Compile hot models after pipeline is created (so f0_model is initialized)
    if vc.config.device.startswith("cuda") and torch.cuda.is_available():
        pipeline.hubert_model = torch.compile(
            pipeline.hubert_model,
            mode="max-autotune",
            dynamic=True,
            fullgraph=False,
        )
        if pipeline.f0_model is not None and hasattr(pipeline.f0_model, "model"):
            # RMVPE: compile the inner nn.Module (E2E)
            target = pipeline.f0_model.model
            if hasattr(target, "model") and isinstance(target.model, torch.nn.Module):
                target.model = torch.compile(
                    target.model,
                    mode="max-autotune",
                    dynamic=True,
                    fullgraph=False,
                )
        elif pipeline.f0_model is not None and hasattr(pipeline.f0_model, "model_arch"):
            # FCPE: compile inner model
            if isinstance(pipeline.f0_model.model_arch, torch.nn.Module):
                pipeline.f0_model.model_arch = torch.compile(
                    pipeline.f0_model.model_arch,
                    mode="max-autotune",
                    dynamic=True,
                    fullgraph=False,
                )
